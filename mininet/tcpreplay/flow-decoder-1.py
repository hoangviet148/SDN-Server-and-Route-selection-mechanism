...
def main():
    consumer_con = pika.BlockingConnection(pika.ConnectionParameters(host=RABBIT_URL))
    consumer_channel = consumer_con.channel()
    consumer_channel.queue_declare(queue=CONSUMER_QUEUE)

    producer_con = pika.BlockingConnection(pika.ConnectionParameters(host=RABBIT_URL))
